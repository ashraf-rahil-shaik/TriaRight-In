...
ini_set('display_errors',1);
    ...
    switch($method) {
        ...
        case "POST":
           ...
           //$stmt->bindParam(':streamName', $user->name);
            ...
            if($user->hasExperience == "")
            {
                ...
                $des="-";
                $trainerdocu="-";
            $sql = "INSERT INTO trainercreation(trainerName,personalEmail,phNumber,Gender,DateOfBirth,aadharNumber,panNumber,aadharImage,panImage,DateOfJoining,qualification,experience,previousOrganization,designation,trainerDocuments,created_at) VALUES(:trainerName,:personalEmail,:phNumber,:Gender,:DateOfBirth,:aadharNumber,:panNumber,:aadharImage,:panImage,:DateOfJoining,:qualification,:experience,:previousOrganization,:designation,:trainerDocuments,:created_at)";
            $stmt = $conn->prepare($sql);
            $created_at = date('Y-m-d');
           //$stmt->bindParam(':streamName', $user->name);
            $stmt->bindParam(':trainerName', $user->trainerName);
        ...
        case "DELETE":
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':trainerId', $path[4]);
